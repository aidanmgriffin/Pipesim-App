    ...
    def disperse(self):
        ...
        while remainingTime > self.manager.tolerance:  # calculate particle movement until flow consumes available time (1 second)
            if self.container.type != "endpoint":
                ...
                # diffusion_rate = self.manager.diffusionCoefficient #D_0
                diffusion_rate = 0.002 #D_0
                asymptotic_diffusion_rate = 0.2

                # print(self.manager.time)
                # diffusion_modified = (asymptotic_diffusion_rate - diffusion_rate)(1 - math.exp())
                ...
                logfile.write("updating particle" + str(self.ID) + "in pipe" + str(self.container.name)  + " time_step = " + self.manager.root.timeStep + "\n")
            elif self.container.type == "endpoint":
